...
import { getArchetypeDialect, type UserArchetype } from '../oracle/archetypeEngine';
import { generateStreakDangerMessage } from '../services/streakProtectionService';
import { getDailyTransitReading, getTransitAdviceForTasks, type DailyTransitReading } from '../services/transitService';
import { generateOracleMessage } from '../services/aiCoachLLM';
import { getCurrentPlanetaryHour, calculateSunTimes, calculateCurrentAspects, findCriticalDegrees, type CurrentAspect, type CriticalDegree } from '../astrology/services/calculations/swissCalculations';
...
import { getAICoachZone, subscribeToAICoachZone } from '../services/aiCoachContextService';
...
function recordMemory(msg: CoachMessage) {
 ...
 */
async function calculateSiderealSolarReturnDate(
...
async function getSolarReturnContext(
...
async function buildOracleContext(
...
// ═══════════════════════════════════════════════════════════════════════════════
// THE VOICE — MESSAGE CANDIDATE TEMPLATES
// ═══════════════════════════════════════════════════════════════════════════════
...
function candidatesFromContext(ctx: OracleContext): CandidateMessage[] {
...
`) },
  ...
  c.push(makeCandidate(`feature-pure-${dateKey}`, {
    ...
    topic: 'feature-pure',
    weight: 34,
    action: { label: 'Try Pure Mode', onClick: () => eventBus.emit('heka-toggle-pure-mode', undefined) },
  }));
  ...
  c.push(makeCandidate(`feature-search-${dateKey}`, {
    ...
    weight: 34,
    action: { label: 'Open Search', onClick: () => eventBus.emit('heka-open-search', undefined) },
  }));
  ...
  c.push(makeCandidate(`feature-stats-${dateKey}`, {
    ...
    topic: 'feature-stats',
    weight: 34,
    action: { label: 'View Stats', onClick: () => eventBus.emit('heka-open-stats', undefined) },
  }));
  ...
  c.push(makeCandidate(`feature-circle-${dateKey}`, {
    ...
    weight: 34,
    action: { label: 'Open Circle', onClick: () => eventBus.emit('heka-open-circle', undefined) },
  }));
  ...
  c.push(makeCandidate(`feature-energy-${dateKey}`, {
    ...
    weight: 34,
    action: { label: 'Cast Vote', onClick: () => eventBus.emit('heka-open-community', undefined) },
  }));
  ...
  c.push(makeCandidate(`feature-year-${dateKey}`, {
    ...
    topic: 'feature-year',
    weight: 34,
    action: { label: 'Open Year View', onClick: () => eventBus.emit('heka-open-year', undefined) },
  }));
  ...
  if (ctx.zone === 'main-calendar') {
    const mainPrompts: PromptTemplate[] = [
      ...
      { text: 'Try a new view. The year modal holds patterns the month view cannot see.', icon: '👁️', topic: 'main-explore', color: '#8b5cf6', action: { label: 'Open Year', onClick: () => eventBus.emit('heka-open-year', undefined) } },
    ...
    if (ctx.timeLabel === 'evening' || ctx.hour >= 19) {
      const eveningPrompts: PromptTemplate[] = [
        { text: 'Evening settles. How are you winding down tonight? The community is sharing energy votes.', icon: '🌇', topic: 'evening-wind-down', color: '#8b5cf6', action: { label: 'Cast Vote', onClick: () => eventBus.emit('heka-open-community', undefined) } },
        ...
        { text: 'Community energy shifts at dusk. See how others are feeling tonight.', icon: '💫', topic: 'evening-community', color: '#f59e0b', action: { label: 'Explore', onClick: () => eventBus.emit('heka-open-community', undefined) } },
  ...
  if (ctx.zone === 'day-panel') {
    const dayPrompts: PromptTemplate[] = [
      ...
      { text: 'You can share this day with your circle. Some moments are better together.', icon: '🔗', topic: 'day-share', color: '#f43f5e', action: { label: 'Share', onClick: () => eventBus.emit('heka-open-circle', undefined) } },
      ...
      { text: 'If this day feels heavy, try Pure Mode. It strips everything back to breath.', icon: '🍃', topic: 'day-pure', color: '#14b8a6', action: { label: 'Enter Pure Mode', onClick: () => eventBus.emit('heka-toggle-pure-mode', undefined) } },
    ];
  ...
  if (ctx.zone === 'circle') {
    const circlePrompts: PromptTemplate[] = [
      ...
      { text: 'A circle of one is still a circle. Add a friend when you are ready.', icon: '👋', topic: 'circle-add', color: '#22c55e', action: { label: 'Invite', onClick: () => eventBus.emit('heka-open-circle', undefined) } },
  ...
  if (ctx.zone === 'year-modal') {
    const yearPrompts: PromptTemplate[] = [
      ...
      { text: 'What is one goal you want to anchor in the next quarter? Write it on a distant month.', icon: '⚓', topic: 'year-goal', color: '#06b6d4', action: { label: 'Plan Ahead', onClick: () => eventBus.emit('heka-open-year', undefined) } },
  ...
  if (ctx.zone === 'community') {
    const communityPrompts: PromptTemplate[] = [
      { text: 'Energy is contagious. Cast your vote and see the collective mood shift.', icon: '💫', topic: 'community-vote', color: '#f59e0b', action: { label: 'Vote', onClick: () => eventBus.emit('heka-open-community', undefined) } },
...
function emitSelectDate(date: HekaDate | null) {
...
function emitAddNote(date: HekaDate | null) {
  const heka = date || civilToHeka(new Date());
  if (heka) {
...
function emitCreateTask(date: HekaDate | null, suggested?: string) {
  const heka = date || civilToHeka(new Date());
  if (heka) {
...
function emitJournal(prompt?: string) {
  eventBus.emit('heka-open-journal', prompt ? { prompt } : {});
  aiConfigService.setUserContext({ lastCoachInteraction: Date.now() });
}

function emitNavigateToStars(tab?: string) {
